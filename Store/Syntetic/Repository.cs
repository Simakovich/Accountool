...
/// <inheritdoc/>
public class GenericSqlRepository<TEntity> : IRepository<TEntity> where TEntity : class, IEntity
{
    ...
    public GenericSqlRepository(IAmbientDbContextLocator ambientDbContextLocator, IDbContextScopeFactory contextScopeFactory, IDbContextScopeContainer dbContextScopeContainer)
    {
    ...
    public IQueryable<TEntity> Get()
    {
        ...
        return option == DbScopeOption.TransactionalReadOnly ? context.Set<TEntity>().AsNoTracking() : context.Set<TEntity>();
    }
    ...
    private void ForEachEntityWithOneChangeTrackerCall(IEnumerable<TEntity> entities, Action<TEntity> entityOperation)
    {
    ...
    private DbContext GetContextWithCurrentDbScopeOptionIfPossible(DbScopeOption expectedOption, out DbScopeOption option)
    {
        var context = _ambientDbContextLocator.Get<ApplicationsDbContext>();
        if (context == null)
        {
            throw new InvalidOperationException($"No ambient DbContext of type {typeof(ApplicationsDbContext)}. Try to check transactions on your services.");
        }

        option = _dbContextScopeContainer.GetDbScopeOption(context);
        if ((byte)option < (byte)expectedOption)
        {
            throw new InvalidOperationException($"Scope of operation is {option} and it's less than expected scope {expectedOption}.");
        }
